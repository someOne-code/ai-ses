...
export const retellToolContracts: RetellToolContract[] = [
  {
    ...
    description:
      "Search active office-scoped listings with structured filters and optional free-text intent. Only use returned listings. Treat search_listings as shortlist-level search output unless a detail field is explicitly present in the returned result. If the tool returns matchInterpretation=verified_structured_match, the results satisfy the structured filters directly. If it returns hybrid_candidate, treat the listings as possible candidates for the free-text intent, not as confirmed proof of subjective criteria such as metroya yakin. If it returns no_match, the requested free-text criterion was not confirmed. If the caller revises or relaxes the search, rebuild the next tool call from the caller's latest active criteria and do not carry over an old free-text intent unless the caller repeats or confirms it. If the caller asks about one selected result's detailed facts such as dues, aidat, building age, floor, elevator, balcony, parking, or address, call get_listing_by_reference with that listing's verified referenceCode before answering instead of guessing from summary search results. Returned listings may include spokenSummary, spokenHighlights, spokenPrice, spokenDues, spokenNetM2, spokenRoomPlan, and spokenReferenceCode for caller-facing speech. Prefer those spoken fields whenever present. Never read raw keys, JSON fragments, tool formatting, field labels, or raw English title text aloud; convert the returned facts into short natural Turkish sentences.",
    parameters: {
      type: "object",
  ...
  },
  {
    ...
    description:
      "Get a single active office-scoped listing by reference code. Preserve the full spoken code including any leading prefix such as DEMO; do not drop tokens. Spacing, hyphen, and case variants are acceptable, but partial codes are not. Use this when the caller asks follow-up detail questions about one selected listing and shortlist search output is not enough. Use the returned fields as source of truth. The verified listing may include spokenSummary, spokenHighlights, spokenPrice, spokenDues, spokenNetM2, spokenRoomPlan, and spokenReferenceCode for caller-facing speech; prefer those spoken fields whenever present. Never read transcript structure, field labels, JSON-like formatting, or raw English title text aloud; summarize the verified listing in short natural Turkish sentences.",
    parameters: {
      type: "object",
      additionalProperties: false,
  ...
  },
  {
    ...
    description:
      "Create a showing request for an office-scoped listing after collecting the minimum required customer details. A usable caller name is enough; do not require surname. The listingId must be the verified backend UUID returned by get_listing_by_reference or another backend tool result, never a raw spoken reference code. Use the confirmed callback number, never a literal placeholder such as {{user_number}}, and only include email if the caller volunteered it. If the caller dictated a new callback number, do not call this tool immediately: first read back the digits in short blocks and ask for explicit confirmation. Only call this tool after that explicit confirmation. When repeating a callback number to the caller, say each digit in short blocks and keep the wording fully Turkish. Do not expose tool names, argument keys, or schema words to the caller.",
    parameters: {
      type: "object",
      additionalProperties: false,
      required: [
